...
export class Messenger {
    ...
    /*
    public sendScanResults(tabID: string, totalIssues: number, securityRecommendationCollection: AggregatedCodeScanIssue[] ){
        let message = '...'
        const filePathList = securityRecommendationCollection.map(recommendation => recommendation.filePath);
const uniqueFilePaths = [...new Set(filePathList)];

const filePathsMessage = uniqueFilePaths.length > 0 
    ? `These issues are found in the following files: <ul>${uniqueFilePaths.map(path => `<li>${path}</li>`).join('')}</ul>`
    : '';
        let x= totalIssues>1? 'are': 'is'
        message = `In this project there ${x} <b>${totalIssues}</b> issues. ${filePathsMessage}`
        this.dispatcher.sendChatMessage(
            new ChatMessage(
                {
                    message,
...
*/
    ...
    public sendUnrecoverableErrorResponse(type: UnrecoverableErrorType, tabID: string) {
        ...
        void vscode.window.setStatusBarMessage('Running Project scan III')
        this.dispatcher.sendChatMessage(
            new ChatMessage(
                {
                    message,
    ...
    public sendTransformationIntroduction(tabID: string) {
        ...
        this.dispatcher.sendAsyncEventProgress(
            new AsyncEventProgressMessage(tabID, {
                inProgress: true,
                message: CodeWhispererConstants.checkingForProjectsChatMessageScans,
                messageId: undefined,
            })
        )
    }

    public sendSecurityScans(tabID: string) {
        this.dispatcher.sendAsyncEventProgress(
            new AsyncEventProgressMessage(tabID, {
                inProgress: true,
                message: undefined,
